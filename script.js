...
function drawMoon(){
    ...
    if (isMobile) {
        ...
        // Màn hình nhỏ: thu nhỏ mặt trăng và đặt hẳn xuống DƯỚI
        // dòng chữ "Happy Birthday" để không bị đè chữ
        const moonScale = Math.max(0.45, viewW / 780);

        moonRadius = 60 * moonScale;
        moonX = viewW - (moonRadius + 30);
        moonY = 130 + moonRadius;

    } else {

        // PC: giữ nguyên y hệt bố cục gốc, không thay đổi
        moonRadius = 60;
        moonX = viewW - 180;
        moonY = 130;
...
function generateHeartShape(){
    ...
    // Thu nhỏ tỉ lệ trái tim trên màn hẹp để không tràn ra ngoài mép
    const heartScale = Math.min(12, (viewW * 0.8) / 32);

    // QUAN TRỌNG: số điểm của đường viền trái tim phải khớp đúng
    // số lượng hearts đang có (kể cả khi giảm bớt trên mobile để
    // tăng hiệu năng), nếu không một đoạn viền sẽ bị thiếu/mất nét
    const totalPoints = Math.max(hearts.length, 60);
...
function playMusic(){
    ...
    .catch(() => {
        ...
        // Trình duyệt (đặc biệt Safari/iOS) chặn autoplay có tiếng
        // khi chưa có thao tác chạm -> giữ trạng thái tắt, chờ user bấm nút
        musicStarted = false;
        musicToggle.textContent = "🔇";
...
// Thử tự phát ngay khi tải trang (sẽ bị chặn trên iPhone cho tới
// khi người dùng chạm vào trang/nút nhạc, đó là giới hạn của trình
// duyệt chứ không phải lỗi code)
playMusic();

// Nếu autoplay bị chặn, chỉ cần người dùng chạm bất kỳ đâu trên
// trang lần đầu là nhạc sẽ tự bật theo (không cần bấm đúng nút)
